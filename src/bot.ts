...
export class ChatGPTBot {
  ...
  checkChatGPTBlockWords(message: string): boolean {
    ...
    let result = config.chatgptBlockWords.some((word) => message.includes(word));
    var endTime = new Date().getTime();

    console.log("Search for Block word takes: " + (endTime - startTime));
    return result;
  }
  ...
  isImportantTalker(talker: ContactInterface): boolean {
    return config.importantTalkers.indexOf(talker.name()) != -1
  }
  ...
  async onMessage(message: Message) {
    ...
    if (!this.isImportantTalker(talker) && this.isNonsense(talker, messageType, rawText)) {
      return;
    }
    if (messageType == MessageType.Audio) {
      if (config.turnOnAudio == false) {
        message.say("抱歉，目前暂不支持。");
        return;
      } else {
        // 保存语音文件
    ...
    if (rawText.startsWith("[收到一条视频/语音聊天消息，请在手机上查看]") ||
      rawText.startsWith("收到红包，请在手机上查看")) {
      message.say("抱歉，目前暂不支持。");
      return;
    }
